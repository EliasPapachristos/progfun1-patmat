...
object Huffman {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith(_._2 < _._2) map {
      case(char: Char, weight) => Leaf(char, weight)
    }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def decode0(myTree: CodeTree, remainingBits: List[Bit], accu: List[Char]): List[Char] = {
        (myTree, remainingBits) match {
          case (myTree: Leaf, Nil) => accu :+ myTree.char
          case (myTree: Leaf, rest) => decode0(tree, rest, accu :+ myTree.char)
          case (myTree: Fork, 0 :: rest) => decode0(myTree.left, rest, accu)
          case (myTree: Fork, 1 :: rest) => decode0(myTree.right, rest, accu)
        }
      }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeChar(myTree: CodeTree, char: Char, accu: List[Bit]): List[Bit] = {
        myTree match {
          case myTree: Leaf => accu
          case myTree: Fork if chars(myTree.left) contains char => encodeChar(myTree.left, char, accu :+ 0)
          case myTree: Fork => encodeChar(myTree.right, char, accu :+ 1)
        }
      }
